            ...
            do // run at least once:
            {
                ...
                combined_shape = pca_model_shape +
                    ...
                    std::abs(Eigen::Map<const VectorXf>(current_pca_coeffs.data(), current_pca_coeffs.size()).norm() -
                             Eigen::Map<const VectorXf>(last_pca_coeffs.data(), last_pca_coeffs.size()).norm()) >=
                    0.01 ||
                    std::abs(
                            Eigen::Map<const VectorXf>(current_blendshape_coeffs.data(),
                                                       current_blendshape_coeffs.size()).norm() -
                            Eigen::Map<const VectorXf>(last_blendshape_coeffs.data(),
                                                       last_blendshape_coeffs.size()).norm()) >= 0.01);
 ...
 */
        ...
        fit_shape_and_pose(const morphablemodel::MorphableModel &morphable_model,
                           ...
                           std::experimental::optional<int> num_shape_coefficients_to_fit, float lambda,
            ...
            return {current_mesh,
                    rendering_params}; // I think we could also work with a Mat face_instance in this function instead of a Mesh, but it would convolute the code more (i.e. more complicated to access vertices).
        };
