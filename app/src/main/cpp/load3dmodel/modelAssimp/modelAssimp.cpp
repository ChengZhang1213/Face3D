...
//    bool isFilesPresent =
//            gHelperObject->ExtractAssetReturnFilename("face3d/out.obj", objFilename) &&
//            gHelperObject->ExtractAssetReturnFilename("face3d/out.mtl", mtlFilename) &&
//            gHelperObject->ExtractAssetReturnFilename("face3d/out.isomap.png", texFilename);
//    if (!isFilesPresent) {
//        MyLOGE("Model %s does not exist!", objFilename.c_str());
//        return;
//    }
    objFilename = modelInfo->objFilePath;
    mtlFilename = modelInfo->mtlFilePath;
    texFilename = modelInfo->imageFilePath;
...
void ModelAssimp::SetViewport(int width, int height) {
    ...
    myGLCamera->SetAspectRatio(ratio);
}
